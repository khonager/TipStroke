...
- Airbrush: two procedural pigment fields—a sparse halo and denser core—with pressure response, hardness-controlled core width, and speed-sensitive deposition.
...
Airbrush uses continuous round geometry modulated by two fixed-scale, sparse pigment textures rather than drawing or stamping blurred circles. The outer coat supplies an irregular halo; hardness changes the width of the denser core; and faster movement lowers deposition. Individual pigment pixels have hard source edges, avoiding the tinted outlines produced by repeatedly compositing anti-aliased circles. The textures use document coordinates, so the permanent result has exactly the canvas pixel resolution and is magnified by the camera like every other raster mark.

Pencil starts with a round contact patch. Pressure changes size and opacity; tilt broadens and flattens it into side shading; barrel orientation rotates that patch; and fast movement can taper size/opacity. Its fixed-scale paper texture is smooth multi-octave value noise with subtle fibers, replacing the prior per-pixel random alpha that looked like television static. Conservative stroke bounds cover the widest rotated tilt mark so sparse tile edges cannot clip it.
