...
- Airbrush: one native blurred raster mask, with pressure-sized geometry and hardness-controlled feathering.
...
Airbrush does not use multiple Ink coats. It follows the same native Android blur technique as the soft eraser: all sampled widths are joined into one filled stroke silhouette, then that unified mask receives one `BlurMaskFilter` pass. This produces one continuous center-to-edge falloff like Procreate's Soft Airbrush, without nested silhouettes or darker overlaps at input-sample boundaries. At 100% opacity the body covers underlying color while the edge fades continuously; hardness reduces the blur radius toward a hard edge. The native wet preview redraws only the current in-memory stroke; pen-up rasterizes only intersecting sparse tiles and creates one bounded undo transaction. The permanent result retains exact canvas-pixel resolution like every other raster mark.
