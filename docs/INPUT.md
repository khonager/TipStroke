...
- One-finger drag can instead smudge the selected paint layer. Smudge transports existing color out of the pickup area before depositing it, so repeated blending cannot create pigment from nothing. It snapshots only affected tiles and groups the whole finger gesture into one undo transaction.
...
- Two or more fingers continue to pan, zoom, and rotate the camera simultaneously; rotation can be locked. After a moved multi-finger gesture loses a pointer, navigation rebases to the remaining finger so a configured one-finger drag can continue without a jump.
